...
# arguemnt names are known/assumed and do not need to be explicity written
ggplot(penguins, 
       aes(x = bill_length_mm, y = bill_depth_mm, color = species)) + # scaling ~ color coding by additional variable, like species 
  geom_point() +
  scale_color_manual(values = c("darkorange", "purple", "cyan4")) # example of how to manually set colors
```
...
ggplot(penguins, aes( x= species, fill = island)) +
  geom_bar() # default position of islands = stacked, which can be hard if you want to color fill the islands by another variable
```
...
# ~ CODE ALONG HERE ~
ggplot(penguins, aes( x= species, fill = island)) +
  geom_bar(position= position_dodge2(preserve = "single")) # makes bars the same width while position is set to dodge 
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar() +
  coord_cartesian() # coord_cartesian is already assumed, but you can include this if you want to inlclude settings within coord function or to change coords 
```
...
ggplot(penguins, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(position = "identity") + #default position is to stack the bars — not overlay!
  scale_fill_manual(values = c("darkorange", "purple", "cyan4"))+
  labs(x = "Fligger Length (mm)", y
        = "Frequency",
       fill = "Species",
